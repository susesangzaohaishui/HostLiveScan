...
const char *host::errno_to_string (int errno_)
{
    switch (errno_) {
...
void host::net_abort(const char *errmsg_)
{
#if defined NET_HAVE_WINDOWS
...
const char *host::wsa_error_no (int no_)
{
...
void host::win_error (char *buffer_, size_t buffer_size_)
{
    unsigned int errcode = GetLastError ();
#if defined _WIN32_WCE
    unsigned int rc = FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM |
        FORMAT_MESSAGE_IGNORE_INSERTS, NULL, errcode, MAKELANGID(LANG_NEUTRAL,
        SUBLANG_DEFAULT), (LPWSTR)buffer_, buffer_size_ / sizeof(wchar_t), NULL );
#else
    unsigned int rc = FormatMessageA (FORMAT_MESSAGE_FROM_SYSTEM |
        FORMAT_MESSAGE_IGNORE_INSERTS, NULL, errcode, MAKELANGID(LANG_NEUTRAL,
        SUBLANG_DEFAULT), buffer_, (unsigned int) buffer_size_, NULL );
#endif
    net_assert (rc);
}

int host::wsa_error_to_errno (int errcode)
{
    switch (errcode) {
