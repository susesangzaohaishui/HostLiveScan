...
#if defined NET_HAVE_OPENVMS
#include <ioctl.h>
#endif

host::fd_t host::open_socket (int domain_, int type_, int protocol_)
{
    //  Setting this option result in sane behaviour when exec() functions
    //  are used. Old sockets are closed and don't block TCP ports etc.
#if defined NET_HAVE_SOCK_CLOEXEC
    type_ |= SOCK_CLOEXEC;
#endif
...
#if !defined NET_HAVE_SOCK_CLOEXEC && defined FD_CLOEXEC
    int rc = fcntl (s, F_SETFD, FD_CLOEXEC);
    errno_assert (rc != -1);
#endif

    //  On Windows, preventing sockets to be inherited by child processes.
#if defined NET_HAVE_WINDOWS && defined HANDLE_FLAG_INHERIT
...
void host::unblock_socket (fd_t s_)
{
#if defined NET_HAVE_WINDOWS
    u_long nonblock = 1;
...
void host::enable_ipv4_mapping (fd_t s_)
{
  (void) s_;
...
bool host::get_peer_ip_address (fd_t sockfd_, std::string &ip_addr_)
{
    int rc;
    struct sockaddr_storage ss;

#if defined NET_HAVE_HPUX || defined NET_HAVE_WINDOWS
