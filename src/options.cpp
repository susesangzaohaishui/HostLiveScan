...
int host::options_t::setsockopt (int option_, const void *optval_,
    size_t optvallen_)
{
    ...
    switch (option_) {
        ...
        /*  Deprecated in favor of NET_IPV6  */
        case NET_IPV4ONLY:
...
//            }
        ...
        case NET_PLAIN_SERVER:
            if (is_int && (value == 0 || value == 1)) {
                as_server = value;
                mechanism = value? NET_PLAIN: NET_NULL;
                return 0;
            }
            break;

        case NET_PLAIN_USERNAME:
            if (optvallen_ == 0 && optval_ == NULL) {
                mechanism = NET_NULL;
                return 0;
            }
...
#       ifdef HAVE_LIBSODIUM
        case NET_CURVE_SERVER:
            if (is_int && (value == 0 || value == 1)) {
                as_server = value;
                mechanism = value? NET_CURVE: NET_NULL;
                return 0;
            }
            break;

        case NET_CURVE_PUBLICKEY:
...
int host::options_t::getsockopt (int option_, void *optval_, size_t *optvallen_)
{
    bool is_int = (*optvallen_ == sizeof (int));
    ...
    switch (option_) {
        ...
        case NET_PLAIN_SERVER:
            if (is_int) {
                *value = as_server && mechanism == NET_PLAIN;
                return 0;
            }
            break;
...
#       ifdef HAVE_LIBSODIUM
        case NET_CURVE_SERVER:
            if (is_int) {
                *value = as_server && mechanism == NET_CURVE;
                return 0;
            }
            break;
