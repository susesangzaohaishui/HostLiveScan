...
host::select_t::handle_t host::select_t::add_fd (fd_t fd_, i_poll_events *events_)
{
    ...
    //  file descriptors.
    net_assert (fds.size () <= FD_SETSIZE);
...
void host::select_t::rm_fd (handle_t handle_)
{
...
void host::select_t::set_pollin (handle_t handle_)
{
    FD_SET (handle_, &source_set_in);
}

void host::select_t::reset_pollin (handle_t handle_)
{
    FD_CLR (handle_, &source_set_in);
}

void host::select_t::set_pollout (handle_t handle_)
{
    FD_SET (handle_, &source_set_out);
}

void host::select_t::reset_pollout (handle_t handle_)
{
    FD_CLR (handle_, &source_set_out);
}

void host::select_t::start ()
{
    worker.start (worker_routine, this);
}

void host::select_t::stop ()
{
    stopping = true;
}

int host::select_t::max_fds ()
{
    return FD_SETSIZE;
}

void host::select_t::loop ()
{
    while (!stopping) {
        ...
        if (retired) {
            fds.erase (std::remove_if (fds.begin (), fds.end (),
                host::select_t::is_retired_fd), fds.end ());
            retired = false;
        }
    }
}

void host::select_t::worker_routine (void *arg_)
{
    ((select_t*) arg_)->loop ();
}

bool host::select_t::is_retired_fd (const fd_entry_t &entry)
{
